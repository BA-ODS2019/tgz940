...
# OPGAVE (a)
# I filen ‘titanic.csv’, som er åbnet med Numbers, kan man anskue følgende datatyper, ‘interger’, ‘float’ og 'string'. 
# Den førstnævnte angiver, at værdien er et helt tal, den næste et decimaltal og sidstnævne et tekststykke. 
...
# Dernæst kan man også se opdelingen af klasser, 1, 2 og 3 (intergers), samt navne på passagerne og deres køn (strings). 
# Til sidst kan man også se deres alder, antal søskende, ægtefæller og forældre (interger) og billetprisen (float).
...
# Dette for-loop giver et klart overblik over alle 8 kategorier. Funktionen gennemløber dataframen og udskriver kategorien for hvert genenmløb. 
for i in df_titanic.columns: 
    print(i)
    
# Her bruger vi shape() til at illustre antallet af kolonner og rækker i dataframen.
...
# Funktionen undersøger og definerer forskellige typer data i dataframen og fortæller, at der findes følgende: Int64, object, float64. 
# Dette kan defineres som henholdsvis integer numbers, strings, og floats. 
...
# Her returnerer funktionen "False", hvilket betyder der ikke eksisterer nogle tomme felter i datasættet
df_titanic.empty
...
# Således ved vi med sikkerhed at tallet '342' symboliserer antallet af overlevende. 
print(df_titanic['Survived'].sum())


# Denne funktion tæller antallet af hver unikke værdi i kolonnen. 
# Her kan vi se både antallet af omkomne(0), hvilket er 545 personer, og antal overlevende(1), 342 personer.   
print(df_titanic['Survived'].value_counts())


# Denne funktion giver overblik over passasegere på hver klasse. Vi har brugt samme funktion som ovenstående, der tæller antallet af hver unikke værdi.
print(df_titanic['Pclass'].value_counts()) 

# Dette er en alternativ måde at se antal rejsende inden for hver klasse med en variabel.
...
# Vi bruger median(), således vi kan se medianen i alder-kategorien.
...
# Her kan vi se gennemsnitsprisen på billetterne ved at bruge mean() funktionen igen. 
...
# Først kan man definere, hvorvidt kolonnen skal opdeles efter en string eller regulært udtryk i Pat.
...
# Vi har lavet et for-loop, der tager udgangspunkt i listen med alle efternavne
...
# aggfunc udgør den type funktion, som man ønsker at anvende, hvilket vi har defineret som count, da vi skal have en optælling af alle rejsende opdelt i klasser. 
...
# I anden klasse var der 97 omkomne
# I trejde klasse var der 368 omkomne
# Dermed var trejde klasse den med flest omkomne.
nyplot = df_titanic.groupby(['Pclass', 'Survived'])['Survived'].count()
print(nyplot)
# Her har vi lavet en visuel fremstilling af pivot-tabellen, med samme plot(kind='bar')-funktionen. 
...
# For at generere pivot-tabellet, vil vi anvende den nye dataframe, værdien af de overlevende, som bliver opstillet i en kolonne sat i forhold til de tre klasser. 
# Denne pivot-tabel bruger sum(), således vi kan se antallet af overlevende. 
# Funktionen finder summen af alle værdierne, bestående af 0 og 1, og fremviser dem i tabellen ud fra hver klasse. 
...
print(tabel1)
# Her har vi igen inkluderet plot(kind=‘bar'), for at demonstrere fordelingen i et søjlediagram. 
vistabel1 = tabel1.plot(kind='bar')


# Det sidste vi vil demonstrere er, hvor mange der i alt overlevede og omkom, ved brug af et for-loop.
...
# Ved hvert for-loop bliver der derfor enten lagt en værdi til 'survived' eller 'not_survived' listerne
# Resultatet af denne viser at vi at der samlet set døde 545 mennesker og kun overlevede 342 (ialt giver dette de 887 passagerer)
survived=0
not_survived=0
